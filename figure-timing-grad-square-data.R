...
for(N in N.vec){
  ...
  for(prediction.order in names(order.list)){
    ...
    m.args <- c(do.sub(Logistic={
    ...
    }), if(N < 1e7)do.sub(Functional.square={
      ...
      loss.values <- quadratic*order.pred.vec^2 + linear*order.pred.vec + constant
      list(
        loss=sum(loss.values[is.loss]),
        gradient=grad.vec)
    }),
    if(N < 1e4)do.sub(Naïve.square={
      is.positive <- N.labels.vec == 1
      pairs.dt <- data.table(expand.grid(
        positive=which(is.positive),
        negative=which(!is.positive)))
      pairs.dt[, positive.pred := order.pred.vec[positive] ]
      pairs.dt[, negative.pred := order.pred.vec[negative] ]
      pairs.dt[, diff := positive.pred-negative.pred ]
      pairs.dt[, diff.loss := margin-diff ]
      ...
      grad.dt <- pairs.tall[, .(
        gradient=sum(grad.sign*2*diff.loss),
        loss=sum(diff.loss^2)
      ), keyby=.(pred.i, label)]
      grad.dt[, pred := order.pred.vec]
      list(
        loss=pairs.dt[, sum(diff.loss^2)],
        gradient=grad.dt[["gradient"]])
    }),
    if(N < 1e7)do.sub(Functional.squared_hinge={
    ...
    if(N < 1e4)do.sub(Naïve.squared_hinge={
      is.positive <- N.labels.vec == 1
      pairs.dt <- data.table(expand.grid(
        positive=which(is.positive),
        negative=which(!is.positive)))
    ...
    if("Naïve.square" %in% names(result.list)){
      stopifnot(with(result.list, all.equal(Naïve.square, Functional.square)))
    }
...
data.table::fwrite(timing.dt, "figure-timing-grad-square-data.csv")
